...
def swap_letters(myword):
    new_word = list(myword)
    mylist = [new_word[0], new_word[1]]
    new_word[0] = new_word[len(new_word) - 2]
    new_word[1] = new_word[len(new_word) - 1]
    new_word[len(new_word) - 2] = mylist[0]
    new_word[len(new_word) - 1] = mylist[1]
    print("Your swapped version of password: " + ''.join(new_word))
    return ''.join(new_word)


def rep_with_upper(myword):
    new_word = list(myword)
    x = randint(0, len(new_word) - 1)
    new_word[x] = new_word[x].upper()
    print("Your upper cased version of password: " + ''.join(new_word))
    return ''.join(new_word)


def passGen(usersnumber):
    while (usersnumber < 3 or usersnumber > 7):
        usersnumber = int(input("You entered wrong value please enter between 3-7: "))

    a = 0
    lastpassword = ""
    passwordList = []
    for a in range(usersnumber):
        passwordList.append(words[randint(0, 1000)])

    for a in passwordList:
        lastpassword = lastpassword + a
    print("Your password is: " + str(lastpassword))
    return lastpassword


def search_letter(searchedSource, searchingKey):
    a = searchedSource.find(searchingKey)
    if a != -1:
        print("The keyword you are looking for is in here.")
        return True
    else:
        print("The keyword you are looking for isn't in here.")
        return False


usersnumber = int(input("Enter a number between 3-7:"))
passwordTemp = passGen(usersnumber)
passwordFinal = swap_letters(rep_with_upper(passwordTemp))
